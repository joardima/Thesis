...
training = 'wdn_reports/Twoloops_SP_672_HTS_5_Noise_2.txt';
...
fw = {1,13,25,73,145,193,289};

%%Read EPANET report - training dataset

[resout, TT, node_ids, link_ids] = read_EPANET_rpt(training,SP,5);
H = resout(1:end,8:14);
H = H';
...
%Read testing data *.mat

load('wntr_sp_7392_dt_5_Nu_2.mat');
...
for i=2:length(fw)
    ...
    H_cv        = H(fw{1}:fw{i},:)';
    D_cv        = D(fw{1}:fw{i},1:6)*1000;
    D_cv        = D_cv';
    Qp_cv       = Q(fw{1}:fw{i},:)*1000;
    Qp_cv       = Qp_cv';
    T           = double(T);
    ...
    err_cv = (Xdot_cv - Xdot_hat_cv(fw{1}:fw{i}-1,:));

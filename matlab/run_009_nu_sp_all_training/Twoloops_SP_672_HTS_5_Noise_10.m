...
[resout, TT, node_ids, link_ids] = read_EPANET_rpt('wdn_reports/Twoloops_SP_672_HTS_5_Noise_10.txt',672,5);
H = resout(1:end,8:14);
H = H';
...
%H    = curr_states{2}; 
node_sensors = find((max(H,[],2)-min(H,[],2)) > 0.1); %1:size(H,1);
...
sel_var = 2; %616 %300 %244; %73 % 735 %371; %73; %232 %616; %390; %616; %300; % % 816 % 4 is a tank

...
[resout, TT, node_ids, link_ids] = read_EPANET_rpt('wdn_reports/Twoloops_SP_168_HTS_15_Noise_10.txt',168,15);
H = resout(1:end,8:14);
H = H';
D = resout(1:end,1:6)/3600; 
D = D';
Q = resout(1:end,22:29)/3600;
Q = Q';
T = TT';
...
% Xdot_hat
tic;
lambda = [0.01:0.01:0.1]; %0.01;      % lambda is our sparsification knob.
nreg = 20;

ERR2 = zeros(0)
j=0
for la=1:length(lambda)
    xi     = sparsifyDynamics(Theta,Xdot,la,nreg);
    toc
...
% Make the estimate (hat) of Xdot
    ...
    [r,c] = size(xi);
    H_elem = xi(H_index(1)+1:H_index(2)+1,1:c);
    H_nzc = nonzeros(H_elem);
    H_nnzc{j} = length(H_nzc);

    D_elem = xi(D_index(1)+1:D_index(2)+1,1:c);
    D_nzc = nonzeros(D_elem);
    D_nnzc{j} = length(D_nzc);

    Q_elem = xi(Q_index(1)+1:Q_index(2)+1,1:c);
    Q_nzc = nonzeros(Q_elem);
    Q_nnzc{j} = length(Q_nzc);
...
H_nnzc = double(cell2mat(H_nnzc));
D_nnzc = double(cell2mat(D_nnzc));
Q_nnzc = double(cell2mat(Q_nnzc));
...
metadata = [min(number_coeff) max(number_coeff) md_coeff md_ERR2_P md_ERR2_D md_ERR2_Q min(H_nnzc) max(H_nnzc) min(D_nnzc) max(D_nnzc) min(Q_nnzc) max(Q_nnzc)];
